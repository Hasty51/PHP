...
1. Создайте функцию `calculateTotalAmount(array $transactions): float`, которая вычисляет общую сумму всех транзакций.
   ![](../images/44_1.png)
    - Выведите сумму всех транзакций в конце таблицы.
     ![](../images/44.png)
3. Создайте функцию `findTransactionByDescription(string $descriptionPart)`, которая ищет транзакцию по части описания.
4. Создайте функцию `findTransactionById(int $id)`, которая ищет транзакцию по идентификатору.
   - Реализуйте данную функцию с помощью обычного цикла `foreach`.![](../images/45.png)
   - Реализуйте данную функцию с помощью функции `array_filter` (на высшую оценку).![](../images/46.png)
5. Создайте функцию `daysSinceTransaction(string $date): int`, которая возвращает количество дней между датой транзакции и текущим днем.
   - Добавьте в таблицу столбец с количеством дней с момента транзакции.
6. Создайте функцию `addTransaction(int $id, string $date, float $amount, string $description, string $merchant): void` для добавления новой транзакции.
...
1. Что такое массивы?
   - Это специальные переменные, которые позволяют хранить несколько значений под одним именем. В отличие от обычной переменной, где лежит одно число или строка, в массив можно «упаковать» целый список данных.
2. Как создать массив?
   - Сейчас чаще всего используют короткий синтаксис с квадратными скобками [], но старый вариант с конструкцией array() тоже работает: php $fruits = ["Apple", "Banana", "Orange"]; // Современный способ $numbers = array(1, 2, 3, 4, 5);          // Классический способ Используйте код с осторожностью.  Также можно создавать ассоциативные массивы, где вместо номеров используются имена (ключи): $user = ["name" => "Ivan", "age" => 25];
3. Зачем нужен foreach?
   - Это самый простой и удобный способ перебрать массив. Цикл по очереди берет каждый элемент массива и позволяет что-то с ним сделать (например, вывести на экран), пока элементы не закончатся. Тебе не нужно знать длину массива или следить за счетчиком — PHP делает всё сам.
